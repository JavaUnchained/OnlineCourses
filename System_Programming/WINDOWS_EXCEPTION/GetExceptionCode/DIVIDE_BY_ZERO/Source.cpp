...
int main()
{
	...
	__except ((GetExceptionCode() == EXCEPTION_INT_DIVIDE_BY_ZERO) ? EXCEPTION_EXECUTE_HANDLER : EXCEPTION_CONTINUE_SEARCH) //значение фильтр
	{
		Log_F("In except. \n");
		Log_F("GetExceptionCode() == EXCEPTION_INT_DIVIDE_BY_ZERO. \n");
		Log_F("Exception execute handalling. \n");
		//код обработчика исключений
		printf("%s", "Divide by zero exception cathed"); // ислючение деления на ноль обработанно
	}
